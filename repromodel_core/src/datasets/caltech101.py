...
class Caltech101Dataset(Caltech101):
    @enforce_types_and_ranges({
        'root': {'type': str, 'default': "repromodel_core/data/caltech101"},
        'target_type': {'type': str, 'default': "category", 'options': ["category", "annotation"]},
    ...
    def __init__(self, root: str, target_type: str = "category", transform: Optional[Callable] = None,
        ...
        self.val_indices = []
        self.test_indices = []
        super().__init__(root=root, target_type=target_type, transform=transform, target_transform=target_transform, download=download)
    ...
    def __getitem__(self, index: int) -> Tuple[Any, Any]:
        ...
        if isinstance(target, int):
            target = one_hot_encode(labels=target, num_classes=101, type="numpy")

        return img, target
    
class _TestCaltech101Dataset(unittest.TestCase):
    def test_initialization(self):
        # Test with default parameters
        dataset = Caltech101Dataset(root="repromodel_core/data/caltech101", download=False)
        self.assertIsInstance(dataset, Caltech101Dataset, "Dataset is not an instance of Caltech101Dataset")
        ...
        dataset = Caltech101Dataset(root="repromodel_core/data/caltech101", target_type="annotation", download=True)
        self.assertEqual(dataset.target_type_str, "annotation", "Custom target_type is not 'annotation'")
        self.assertTrue(dataset.downloaded, "Custom download is not True")
        # Check if the dataset files exist
        data_dir = os.path.join("repromodel_core/data/caltech101", "caltech101")
    ...
    def test_set_mode(self):
        dataset = Caltech101Dataset(root="repromodel_core/data/caltech101")
    ...
    def test_generate_indices_and_set_fold(self):
        ...
        with self.assertRaises(RuntimeError, msg="Setting fold without generating indices did not raise an error"):
            dataset_no_indices = Caltech101Dataset(root="repromodel_core/data/caltech101")
